...
class TimeParser:
    def __init__(self):
        ...
        # Регулярные выражения для русского языка (УЛУЧШЕННЫЕ)
        self.ru_patterns = {
            # Относительное время
            ...
            # Простое время с предлогом
            'simple_time_ru': r'^в\s+(\d{1,2})(?:[:\.](\d{2}))?\s*(утра|вечера|ночи|дня)?$',
            
            # Время без предлога
            'time_no_prep': r'^(\d{1,2})(?:[:\.](\d{2}))?\s*(утра|вечера|ночи|дня)$',
        }
        
        # Регулярные выражения для английского языка (УЛУЧШЕННЫЕ)
        self.en_patterns = {
            # Relative time
            ...
            # Time without "at" but with AM/PM
            'time_ampm': r'^(\d{1,2})(?::(\d{2}))?\s*(AM|PM|am|pm)$',
        }
        
        # Словари для конвертации (без изменений)
        self.ru_months = {
            'января': 1, 'январю': 1,
            'февраля': 2, 'февралю': 2,
            'марта': 3, 'марту': 3,
    ...
    def _parse_ru(self, time_str: str, base_time: datetime, timezone: str) -> Tuple[Optional[datetime], str, Dict]:
        """Парсинг для русского языка"""
        user_tz = pytz.timezone(timezone)
        
        # 1. Относительное время: "через X часов"
        ...
        if match:
            ...
            hour = self._adjust_hour_ru(hour, time_of_day)
            result = base_time + timedelta(days=2)  # ИСПРАВЛЕНО: было days=1
            result = result.replace(hour=hour, minute=minute, second=0, microsecond=0)
        ...
        # 11. Простое время с предлогом: "в 8 утра"
        ...
        # 12. Время без предлога но с указанием времени суток: "8 утра"
    ...
    def _parse_en(self, time_str: str, base_time: datetime, timezone: str) -> Tuple[Optional[datetime], str, Dict]:
        """Парсинг для английского языка"""
        user_tz = pytz.timezone(timezone)
        
        # 1. Relative time: "in X hours"
        ...
        # 5. "day after tomorrow at XX:XX AM/PM" - ИСПРАВЛЕНО
        ...
        # 11. Simple time with "at": "at 8 AM"
        ...
        # 12. Time with AM/PM but without "at": "8:00 PM"
    ...
    def extract_reminder_text(self, full_text: str, language: str = 'ru') -> Tuple[str, str]:
        ...
        if language == 'ru':
            patterns = [
                r'(.*?)(?:^|\s)(через\s+\d+\s+(?:час|минут|день|недел).*?)(?:\s|$|\.|!)',
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                # Очищаем время от лишних предлогов
                if language == 'ru':
    ...
    def detect_repeat_pattern(self, time_str: str, language: str = 'ru') -> Dict[str, Any]:
        ...
        ru_patterns = {
            'daily': [r'каждый день', r'ежедневно', r'каждое утро', r'каждый вечер'],
            'weekly': [r'каждую неделю', r'по понедельникам', r'по вторникам', r'по средам',
                      r'по четвергам', r'по пятницам', r'по субботам', r'по воскресеньям',
                      r'по будням', r'по выходным'],
            'monthly': [r'каждый месяц', r'ежемесячно'],
            'yearly': [r'каждый год', r'ежегодно']
        }
        
        en_patterns = {
            'daily': [r'every day', r'daily', r'each day', r'every morning', r'every evening'],
            ...
            'yearly': [r'every year', r'yearly', r'annually', r'each year']
        }
